...
function host_config_to_html(
  ...
  const base_host_scheme = `
    <div class="row">
      ...
      <input class="form-control" type="text" name="mac" id="${id}_mac" value="${
    host_or_new.mac_addr
  }"/>
      </div>
      <div class="col-4 col-md input-group">
      <label class="input-group-text" for="${id}_ip">Ip</label>
      <input class="form-control" type="text" name="ip"  id="${id}_ip" value="${
    host_or_new.ip_addr || ''
  }" />
      </div>
      <div class="col-4 col-md input-group">
      <label class="input-group-text" for="${id}_name">Name</label>
      <input class="form-control" type="text" name="name"  id="${id}_name" value="${
    host_or_new.name || ''
  }" />
      </div>
    </div>
  ...
  const started_icon =
    '<span class="material-symbols-outlined input-group-text" style="color:green">radio_button_checked</span>';
  // const started_icon = '<span class="material-symbols-outlined" style="color:green">screen_record</span>'
  const unknown_icon =
    '<span class="material-symbols-outlined input-group-text" style="color:gray">help</span>';
  const starting_icon =
    '<span class="material-symbols-outlined input-group-text" style="color:grey">radio_button_partial</span>';
  const stopped_icon =
    '<span class="material-symbols-outlined input-group-text" style="color:grey">radio_button_unchecked</span>';
  const error_icon =
    '<span class="material-symbols-outlined input-group-text" style="color:red">running_with_errors</span>';
  ...
  if (new_host_line)
    status_icon =
      '<span class="material-symbols-outlined input-group-text" style="color:grey">add_to_queue</span>';
...
function host_config_to_htmlnode(
  host: HostConfig,
  ws: WebSocket,
  new_host_line: boolean
) {
  ...
  if (new_host_line) {
    ...
    const new_btn = new_node.querySelector<HTMLButtonElement>(
      `[id="${mac_text}_btn_add"]`
    );
    const new_ip_text = new_node.querySelector<HTMLInputElement>(
      `[id="${mac_text}_ip"]`
    );
    const new_mac_text = new_node.querySelector<HTMLInputElement>(
      `[id="${mac_text}_mac"]`
    );
    const new_name_text = new_node.querySelector<HTMLInputElement>(
      `[id="${mac_text}_name"]`
    );
    if (new_btn === null) {
    ...
    new_btn.addEventListener('click', () => {
      ...
      if (!mac_addr) throw Error('Cannot have empty mac address');
    ...
    fields.forEach((field) => {
      field?.addEventListener('input', (err) => {
        ...
        } else if (!new_btn.hasAttribute('disabled'))
          new_btn.setAttribute('disabled', 'true');
      });
    });
  } else {
    const mac_text = mac_to_id(host.mac_addr);
    const wol_btn = new_node.querySelector<HTMLButtonElement>(
      `[id="${mac_text}_btn_wake"]`
    );
    const rem_btn = new_node.querySelector<HTMLButtonElement>(
      `[id="${mac_text}_btn_remove"]`
    );
    const save_btn = new_node.querySelector<HTMLButtonElement>(
      `[id="${mac_text}_btn_save"]`
    );
    ...
    save_btn.addEventListener('click', () => {
      if (!new_mac_text?.value) throw Error('Cannot have empty mac address');
    ...
    fields.forEach((field) => {
      ...
      field?.addEventListener('input', (_event) => {
        const mac_addr = new_mac_text?.value;
        const ip_addr = new_ip_text?.value;
        const name = new_name_text?.value;
        ...
        } else if (!save_btn.hasAttribute('disabled'))
...
function refresh_hosts(
  ...
  for (let i = 0; i < cl && i < hosts.length; i++) {
    ...
    const host = hosts[i];
    if (overwrite_mac) {
      if (overwrite_mac !== host.mac_addr) continue;
    }
  ...
  if (dl < 0) {
    ...
    for (let i = dl; i > 0; i--) {
      const c_t_remove = childs[hl_total + i - 2];
      sceneData.hostsList.removeChild(c_t_remove);
    }
  }
}

function manage_msg_client(
  ws: WebSocket,
  ...
  switch (msg.kind) {
    ...
    case 'EndPing':
      {
        ...
        const hostidx = sceneData.hostDataList.findIndex(
    ...
    case 'RefreshHosts':
      sceneData.hostDataList = common_hosts_to_client(
        msg.data.hosts,
        sceneData.hostDataList
      );
...
(() => {
  const ws = new WebSocket(
    `ws://${window.location.hostname}:${SERVER_WS_PORT.toString()}`
  );
  ...
  ws.addEventListener('message', (event) => {
    ...
    try {
      ...
      if (!('data' in event && typeof event.data === 'string'))
        throw Error('wrong (or no) "data" field in message!');
      const parsed: unknown = JSON.parse(event.data);
      if (!common.isToClientMessage(parsed))
        throw Error('message is not right');
